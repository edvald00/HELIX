      ...
      <header className="sticky top-0 z-50 h-[72px] backdrop-blur-md bg-[#0b1120]/80 border-b border-[#1E293B]">
        <nav className="max-w-[1400px] mx-auto px-6 h-full flex items-center justify-between gap-6">
          ...
          <div className="flex items-center">
            <span className="text-[#94A3B8] text-[14px] mr-4 hidden sm:block">{userName}</span>
            <Link href={returnUrl} className="inline-flex items-center gap-2 px-[18px] py-[10px] rounded-[10px] text-[14px] font-semibold border border-[#00AEEF] text-[#00AEEF] hover:bg-[#00AEEF]/10 transition-all">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {ROOMS.map((room) => (
            <Link
              ...
              key={room.id}
              className={`group relative h-80 rounded-2xl overflow-hidden border ${room.border} flex flex-col justify-end p-6 transition-all hover:-translate-y-1 hover:shadow-[0_10px_30px_rgba(0,174,239,0.15)] bg-slate-900`}
            >
              <Image
                src={room.image}
                alt={room.name}
              ...
              <div className="absolute top-4 left-4 flex items-center gap-2 bg-black/50 backdrop-blur px-3 py-1.5 rounded-full border border-white/10">
                <span className={`w-2 h-2 rounded-full ${room.statusColor} ${room.status === "Sistema Online" ? "animate-pulse" : ""}`} />
              ...
              <div className="relative z-10">
                ...
                <p className="text-sm text-gray-300 group-hover:text-white transition-colors">Abrir painel 3D</p>
              </div>
            </Link>
          ))}
        </div>
